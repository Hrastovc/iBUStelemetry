...
/* static */ 
inline void iBUStelemetry::tunedDelay(uint16_t delay) { 
  _delay_loop_2(delay);
}

// This function sets the current object as the "listening"
// one and returns true if it replaces another 
bool iBUStelemetry::listen()
{
...
bool iBUStelemetry::stopListening()
{
  if (active_object == this)
  {
    setRxIntMsk(false);
...
uint8_t iBUStelemetry::rx_pin_read()
{
...
/* static */
inline void iBUStelemetry::handle_interrupt()
{
  if (active_object)
  {
    active_object->recv();
...
// Constructor
//
iBUStelemetry::iBUStelemetry(uint8_t receivePin, uint8_t transmitPin, bool inverse_logic /* = false */, bool full_duplex /* = true */) : 
...
iBUStelemetry::iBUStelemetry(uint8_t receivePin) : 
  _rx_delay_centering(0),
  _rx_delay_intrabit(0),
  _rx_delay_stopbit(0),
  _tx_delay(0),
...
void iBUStelemetry::setTX(uint8_t tx)
{
...
void iBUStelemetry::setRX(uint8_t rx)
{
  pinMode(rx, INPUT);
  if (!_inverse_logic)
...
uint16_t iBUStelemetry::subtract_cap(uint16_t num, uint16_t sub) {
  if (num > sub)
    return num - sub;
  else
    return 1;
...
void iBUStelemetry::begin(long speed)
{
...
void iBUStelemetry::setRxIntMsk(bool enable)
{
    if (enable)
...
size_t iBUStelemetry::write(uint8_t b)
{
  if (_tx_delay == 0) {
    setWriteError();
    return 0;
      ...
      switch(msg){
        case 0b1000:
          ...
          if(adr <= sensorCount){
            write(0x04);
            write((0b1000 << 4) + adr);
            checksum = 0xFFFF - (0x04 + ((0b1000 << 4) + adr));
            write(checksum);
            write(checksum >> 8);
          }
          break;
        case 0b1001:
          ...
          write(0x06);
          write((0b1001 << 4) + adr);
          write(sensorType[adr]);
          write(0x02);
          checksum = 0xFFFF - (0x06 + ((0b1001 << 4) + adr) + sensorType[adr] + 0x02);
          write(checksum);
          write(checksum >> 8);
          break;
        case 0b1010:
          ...
          write(0x06);
          write((0b1010 << 4) + adr);
          write(sensorValue[adr]);
          write(sensorValue[adr] >> 8);
          checksum = 0xFFFF - (0x06 + ((0b1010 << 4) + adr) + (sensorValue[adr] & 0b11111111) + (sensorValue[adr] >> 8));
          write(checksum);
          write(checksum >> 8);
          break;
      }
...
void iBUStelemetry::addSensor(byte type){
  sensorCount++;
